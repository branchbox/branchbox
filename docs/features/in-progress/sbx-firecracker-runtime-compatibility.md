...
- [ ] **Coding-agent identity lease (both):** the demo had to copy only the authenticated Codex `auth.json` from an existing user-owned devcontainer into the disposable SBX devcontainer, repair the root-owned named-volume directory, and enforce directory mode `0700` plus file mode `0600`. Replace that direct copy with an account/project/user-or-agent scoped credential lease. BranchBox should accept an opaque lease/materialization instruction and report only the credential name, consumer, scope, expiry, and revocation state; Agentify owns authorization and secret retrieval. Firecracker must materialize the same lease inside the guest/devcontainer without mounting a persistent human home and must revoke and erase it on teardown.

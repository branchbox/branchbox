...
# Source persisted 1Password references from branchbox init (if available).
...
if [[ -f "${DEVCONTAINER_OP_ENV}" ]]; then
  _saved_github="$(grep '^OP_GITHUB_REF=' "${DEVCONTAINER_OP_ENV}" | cut -d= -f2- || true)"
  _saved_signing="$(grep '^OP_SIGNING_KEY_REF=' "${DEVCONTAINER_OP_ENV}" | cut -d= -f2- || true)"
  if [[ -n "${_saved_github}" && "${OP_GITHUB_REF}" == "${DEFAULT_OP_GITHUB_REF}" ]]; then
    OP_GITHUB_REF="${_saved_github}"
  fi
  if [[ -n "${_saved_signing}" && "${OP_SIGNING_KEY_REF}" == "${DEFAULT_OP_SIGNING_KEY_REF}" ]]; then
    OP_SIGNING_KEY_REF="${_saved_signing}"
  fi
fi

...
- Publish user-facing documentation with Docusaurus. Source files live under `docs/docs/`, configuration under `docs/docusaurus.config.js`, and shared assets inside `docs/static/`; keep specs automation untouched in `docs/features/`.
- Install docs dependencies with `npm install` inside `docs/` (the devcontainer includes Node.js and npm). Use `npm run start` for local preview and `npm run build` for production checks.
- CI must include a fast `npm run build` (or equivalent `docusaurus build`) check on PRs. A dedicated Pages workflow should deploy the `docs/build/` output to `gh-pages` on successful pushes to `main`.
- Keep CLI reference pages generated: `docs/scripts/render-cli-reference.sh` captures `branchbox --help` output into `docs/docs/reference/cli.md`. Regenerate during releases or when command flags change.
- Engineers and coding agents must update the sidebar configuration (`docs/sidebars.js`) whenever new guides are added, mirror critical entry points in `README.md`, and document any automation adjustments in this file so future contributors know how docs are built and shipped.
